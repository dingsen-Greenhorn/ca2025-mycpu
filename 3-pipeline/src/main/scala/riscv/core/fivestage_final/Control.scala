...
class Control extends Module {
  ...
  // A: The data from a load instruction is needed to take from the end the Mem-stage, however 
  //    the dependent insturuction must to have the date immediately for Ex-stage.while Forwarding cannot
  //    time travel so only stall can do the work.
  //
  // Hint: Consider data dependency and forwarding limitations
  //
  // Q2: What is the difference between "stall" and "flush" operations?
  // A: [stall] Stop the pipeline stage and PC, hold the current instruction to wait for dependency.(Insert a bubble into next stage)
  //    [flush  Reset the pipeline reg to default stage. delete the instruction should not be used.
  // Hint: Compare their effects on pipeline registers and PC 
  //
  // Q3: Why does jump instruction with register dependency need stall?
  // A: In some design the, we optimize the jumps/branchs instruction in the ID stage(rather than EX stage) if a Jalr instruction
  //    needs a register value to calculate the target address, and which is cuurent update in Ex-stage, the result isn't ready for ID-stage
  //    we need to stall and wait.
  // Hint: When is jump target address available?
  //
  // Q4: In this design, why is branch penalty only 1 cycle instead of 2?
  // A: The final_fivestage design, it can know the branch in ID-stage,If taken only IF-stage is wrong.compare to standard desigen,which will 
  //    be wrong ID and IF two stage.
  ...
  // A: The core will suffer from data Hazard and exe incorrectly.
  //    - Instruction will read the old state value, for won't wait the lord and the forwording.
  //    - Exe error will comes cause the Jalr address is wrong, for the reg sourse date is not ready.  
  // Hint: Consider data hazards and control flow correctness
  //
  // Q6: Complete the stall condition summary:
  // Stall is needed when:
  // 1. A Lord-use hazard is exist or ID dependency will the needed data is at Ex-stage (EX stage condition)
  // 2. An ID depends on a load instruction currently in the MEM stage (MEM stage condition)
  //
  // Flush is needed when:
  // 1. A branch is taken or a jump, the IF or ID need to reset.  (Branch/Jump condition)
  //
}
